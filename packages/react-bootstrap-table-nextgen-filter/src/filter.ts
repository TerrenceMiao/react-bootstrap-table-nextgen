...
export const filterByText =
  ...
  (
    data: any,
    dataField: any,
    { filterVal: userInput = "", comparator = LIKE, caseSensitive }: any,
    ...
    return data.filter((row: any) => {
      ...
      return (
        cellStr.toLocaleUpperCase().indexOf(filterVal.toLocaleUpperCase()) !==
        -1
      );
    });
  };
...
export const filterByNumber =
  ...
  (
    data: any,
    dataField: any,
    { filterVal: { comparator, number } }: any,
    customFilterValue: any
  ) =>
    data.filter((row: any) => {
...
export const filterByArray =
  (_: any) =>
  (data: any, dataField: any, { filterVal, comparator }: any) => {
    if (filterVal.length === 0) return data;
    const refinedFilterVal = filterVal
      .filter((x: any) => _.isDefined(x))
    ...
    return data.filter((row: any) => {
      ...
      return refinedFilterVal.some(
        (item: any) => cellStr.indexOf(item.toLocaleUpperCase()) !== -1
      );
    });
  };

export const filterFactory = (_: any) => (filterType: any) => {
  switch (filterType) {
...
export const filters =
  (data: any, columns: any, _: any) =>
  (currFilters: any, clearFilters: { [dataField: string]: any } = {}) => {
